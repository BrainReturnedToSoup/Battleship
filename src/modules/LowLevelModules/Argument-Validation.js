export class ArgumentValidation {
  ...
  #argValidator(methodName, argsObj) {
    if (this.#validationRules.hasOwnProperty(methodName)) {
      const methodValidationData = this.#validationRules[methodName];
  ...
  validate(methodName, argsObj) {
    this.#argValidator(methodName, argsObj);
  }
}
